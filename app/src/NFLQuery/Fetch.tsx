...
function getGames(year: number): Promise<GameType[]> {
  return fetchCompletedGameIds(year)
    .then((gameIds) => gameIds.filter((gameId) => !EXCLUDED_GAME_IDS.has(gameId)))
    .then((gameIds) => Promise.all(gameIds.map((gameId) => fetchSummaryGame(gameId))));
}
...
export async function fetchSummaryGame(gameId: number): Promise<GameType> {
  ...
  try {
    ...
    return {
      ...game,
      scores:
        game.drives[game.drives.length - 1]?.scores ?? ([0, 0] as [
          number,
          number,
        ]),
    };
...
function convertSummaryToGame(
  ...
  return {
    ...
    week:
      summary.header.week *
      (summary.header.season.type === 3 ? -1 : 1),
    teams,
    drives,
    scores: [0, 0],
  };
